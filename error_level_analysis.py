...
COLORMAP: int = cv2.COLORMAP_MAGMA  # фиолетово‑оранжевый, как раньше
VIS_P_LO: int = 2                   # p2..p98 — робастная растяжка
VIS_P_HI: int = 98
ELA_GAIN: float = 1.6               # лёгкое усиление после растяжки
ELA_GAMMA: float = 0.9              # гамма <1 — подчёркиваем низкие уровни
ELA_USE_CLAHE: bool = False         # отключаем CLAHE, чтобы оставить «зерно»
...
def make_ela_visual(ela_u8: np.ndarray,
    ...
    """
    «Старый» стиль:
    - робастная растяжка по перцентилям (p2..p98);
    - НЕТ CLAHE/сглаживания, сохраняем «зерно»;
    - MAGMA LUT для фиолетово‑оранжевого вида.
    """
    ...
    # лёгкий gain + гамма < 1 — поднимаем «темноту» для заметности шума
...
def acid_overlay_on_original(pil_img: Image.Image,
    ...
    # оставляем без прямоугольников, если не нужны — но код рисования пусть будет:
